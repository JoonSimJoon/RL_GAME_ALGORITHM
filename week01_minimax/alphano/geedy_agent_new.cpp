...
mt19937 r1(123);
uniform_int_distribution<int> r2(0,1e9);
...
{
    int i,j,s=0;
    for(i=1; i<=7; i++)for(j=1; j<=7; j++)s+=b[i*8+j]==turn?1:b[i*8+j]==0?0:-1;
    return s;
}

int ap(tuple<int, int, int, int> t)
{
    auto [x1, y1, x2, y2]=t;
    int b[8][8],i,j;
    if (x1 == -1 && y1 == -1 && x2 == -1 && y2 == -1) return -1e9;
    for(i=0; i<7; i++)for(j=0; j<7; j++)b[i+1][j+1]=board[i+1][j+1];
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        if (b[nx][ny] == (turn ^ 3)) b[nx][ny] = turn;
    }
    return eval(b[0]);
}
...
{
    tuple<int, int, int, int> a,m= {-1,-1,-1,-1};
    int b=0,i;
