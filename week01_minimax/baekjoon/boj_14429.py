...
def solve():
    # 입력: A명의 플레이어, B번째 사람이 시작
    A, B = map(int, input().split())

    # 시뮬레이션: 각 플레이어가 최적으로 플레이한다고 가정
    # 패배 포지션: 3, 7, 11, 15, 19, 23, 27, 31
    # 각 플레이어는 상대방을 패배 포지션에 놓으려고 함
    ...
    while current_number <= 31:
        ...
        # 다음 패배 포지션 찾기
        next_lose = ((current_number // 4) + 1) * 4 - 1

        if next_lose > 31:
            next_lose = 31

        # 현재 플레이어가 말할 개수
        count = next_lose - current_number + 1
