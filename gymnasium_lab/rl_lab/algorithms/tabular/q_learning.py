...
class QLearningAlgorithm(RLAlgorithm):
    ...
    def train(self, run_dir: Path) -> dict[str, Any]:
        ...
        try:
            ...
            for episode in range(1, int(self.config["train_episodes"]) + 1):
                ...
                while not (done or truncated):
                    ...
                    next_state, reward, done, truncated, _ = env.step(action)
                    best_next_q = 0.0 if (done or truncated) else float(np.max(self.q_table[int(next_state)]))
                    td_target = float(reward) + float(params["gamma"]) * best_next_q
        ...
        plot_training_curve(
            ...
            title=f"{self.env_spec.key} - Q-Learning",
